...
def _open_split_submenu(win: Window) -> UiaElement:
    ...
    send_keys("{RIGHT}")
    item = _focus_settles(
        lambda e, n=entry.name: e.name != n and (e.class_name in ("AppBarButton", "MenuFlyoutSubItem", "MenuFlyoutItem")),
        timeout=3.0,
    )
    assert item.name != entry.name and (item.class_name in ("AppBarButton", "MenuFlyoutSubItem", "MenuFlyoutItem")), (
...
def test_the_measurement_can_see_a_split(terminal):
    ...
    if item.class_name == "MenuFlyoutSubItem":
        # In MenuFlyout (#20612), subitems expand; the first child is the explicit Automatic split item.
        send_keys("{RIGHT}")
        _focus_settles(lambda e: e.class_name == "MenuFlyoutItem", timeout=3.0)
    send_keys("{ENTER}")
